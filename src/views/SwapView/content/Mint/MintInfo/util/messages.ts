export default {
  stabilityFee: {
    en: 'Stability fee',
    ru: 'Стабилизационный сбор',
    fr: 'Frais de stabilité',
    es: 'Tarifa de estabilidad',
    pt: 'Taxa de estabilidade',
    de: 'Stabilitätsgebühr',
    zh: '稳定费',
  },
  conversionRate: {
    en: 'Conversion rate',
  ...
  tooltip: {
    stabilityFee: {
      en: `
      The StakeWise protocol collects a {percent}% stability fee on {token} rewards.
      The fee is added to the total amount of {token} minted by the user.
    `,
      ru: `
      Протокол StakeWise взимает {percent}% комиссию за стабильность с вознаграждений {token}.
      Комиссия добавляется к общей сумме токенов {token}, заминченных пользователем.
    `,
      fr: `
